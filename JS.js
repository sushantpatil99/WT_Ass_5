 ...
 */
let base_url = "http://localhost/WT_ASSS_5/handler.php";
var today;
var reg = [];
var details = {};
var num = 0;
...
function validateForm()
{
    ...
    if (check===0)
    {
        ...
        num = num +1;
        var obj = JSON.stringify({ srno: num, name: name, email: email, contact: mobile, password: password, type: eventtype, requirement: requirements, otherreq: otherrequirements, space:eventspace, date: eventdate, slot: timeslot, timestamp: timestamp });
...
function updateTable()
{
    ...
    $.get(base_url + "?req=getD", function(data, success) {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                c1.innerHTML = i+1;
                c2.innerHTML = data[i].timestamp;
                c3.innerHTML = data[i].name;
                c4.innerHTML = data[i].email;
                c5.innerHTML = data[i].contact;
                c6.innerHTML = data[i].type;
                c7.innerHTML = data[i].requirement;
                c8.innerHTML = data[i].otherreq;
                c9.innerHTML = data[i].space;
                c10.innerHTML = data[i].date;
                c11.innerHTML = data[i].slot;
